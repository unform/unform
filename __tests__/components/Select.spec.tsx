...
describe('Form', () => {
  it('should display label', () => {
    const {getByText} = render(
      <Form onSubmit={jest.fn()}>
        <Select options={[{id: 'node', title: 'NodeJS'}]} name="tech" label="Tech" />
      </Form>,
    )
  ...
  it('should display error on blur', async () => {
    const schema = Yup.object().shape({
      tech: Yup.string().required('Tech is required'),
    })
    ...
    const {getByText, getByLabelText} = render(
      <Form schema={schema} onSubmit={jest.fn()}>
        <Select options={[{id: 'node', title: 'NodeJS'}]} name="tech" label="Tech" validateOnBlur />
      </Form>,
    )

    act(() => {
      fireEvent.blur(getByLabelText(/tech/i))
    })
  ...
  it('should return correct selected value', () => {
    ...
    const {getByTestId, getByLabelText} = render(
      <Form onSubmit={submitMock}>
        <Select options={[{id: 'node', title: 'NodeJS'}, {id: 'react', title: 'ReactJS'}]} name="tech" label="Tech" />
      </Form>,
    )
