...
- `ref`: Input ref.
- `getValue`: The method used to return the input value. In a simple input, we just need to return `ref.current.value`;
- `setValue`: Changes the input value when you use the `formRef.current.setFieldValue` method;
- `clearValue`: If you plan to use the reset propery, this method will handle the logic to clear the value.
